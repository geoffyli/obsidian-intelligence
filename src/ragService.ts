...
export class RAGService {
	...
	private async saveVectorStoreData(): Promise<void> {
		...
		try {
            ...
            const documentsToSave: Array<Pick<Document, "pageContent" | "metadata">> = []; // Changed DocumentInterface to Document
			...
			new Notice(`RAG Vector Store saved to ${filePath}`);
			console.log(
				`RAG Vector Store saved with ${data.documents.length} items.`
			);
		} catch (error) {
			console.error("Error saving RAG vector store:", error);
			new Notice("Error saving RAG vector store. Check console.");
		}
	}

    private async loadVectorStoreData(): Promise<boolean> {
        if (!this.embeddingsService) {
            console.error("Embeddings service not initialized before loading vector store.");
            return false;
        }
        await this.ensureDataDirExists();
        const filePath = `${this.dataDir}/${VECTOR_STORE_FILE_PATH}`;
        ...
        try {
            ...
            if (!data.documents || !data.vectors || data.documents.length !== data.vectors.length) {
            ...
            this.allDocumentChunks = data.documents.map(docData => new Document(docData));
            
            // Reconstruct MemoryVectorStore
            this.mainVectorStore = new MemoryVectorStore(this.embeddingsService);
            await this.mainVectorStore.addVectors(data.vectors, this.allDocumentChunks);

            new Notice(`RAG Vector Store loaded with ${this.allDocumentChunks.length} documents.`);
            console.log(`RAG Vector Store loaded from ${filePath} (Persisted at: ${new Date(data.persistedAt).toLocaleString()})`);
            return true;
        } catch (error) {
            console.error("Error loading RAG vector store:", error);
            new Notice("Error loading RAG vector store. Will re-index. Check console.");
            // Attempt to remove potentially corrupted file
            try { await this.app.vault.adapter.remove(filePath); } catch (e) { /* ignore */ }
            return false;
        }
    }
    ...
    async initialize(): Promise<void> {
        ...
        if (!this.settings.openAIApiKey) {
         ...
         if (!this.plugin || !this.dataDir) {
            console.error("RAGService: Plugin reference not set. Call setPlugin() first.");
            new Notice("RAG Plugin error: internal setup incomplete.");
            return;
        }


        this.isInitializing = true;
        if (this.plugin) this.plugin.updateStatusBar("RAG: Initializing...");
        ...
        if (!loadedFromPersistence) {
            if (this.plugin) this.plugin.updateStatusBar("RAG: Indexing vault...");
            new Notice("No valid persisted data. Indexing vault...");
            try {
                this.allDocumentChunks = await loadVaultDocuments(this.app);
                if (!this.allDocumentChunks || this.allDocumentChunks.length === 0) {
                    new Notice("No markdown documents found in the vault to index.");
                    if (this.plugin) this.plugin.updateStatusBar("RAG: No docs");
                    this.isInitialized = false; this.isInitializing = false; return;
                }
                
                if (this.plugin) this.plugin.updateStatusBar(`RAG: Embedding ${this.allDocumentChunks.length} chunks...`);
                if (!this.embeddingsService) throw new Error("Embeddings service not available for indexing.");
                
                this.mainVectorStore = await createAndEmbedVectorStore(this.allDocumentChunks, this.embeddingsService);
                await this.saveVectorStoreData(); // Save after successful indexing

            } catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
                console.error("Error during initial vault indexing:", errorMessage);
                new Notice(`RAG Indexing Failed: ${errorMessage}.`);
                if (this.plugin) this.plugin.updateStatusBar("RAG: Indexing Failed");
                this.isInitialized = false; this.isInitializing = false; return;
            }
        }
        ...
        if (this.llm && this.mainVectorStore) {
            this.chain = await createConversationalRAGChain(this.llm, this.mainVectorStore);
            this.isInitialized = true;
            if (this.plugin) this.plugin.updateStatusBar("RAG: Ready");
            new Notice("RAG Service Initialized Successfully!");
        } else {
            new Notice("RAG Service initialization failed: LLM or Vector Store missing.");
	 ...
	 */
	private createContextFromFilteredDocuments(docs: Document[], appliedFilters?: MetadataFilter[]): string | null {
		if (!docs || docs.length === 0) {
			return null;
		}

		// Get unique file names and basic metadata
		const uniqueFiles: Record<string, { path: string; modifiedAt?: number; createdAt?: number }> = {};
		docs.forEach((doc) => {
			if (doc.metadata.source && !uniqueFiles[doc.metadata.source as string]) {
				uniqueFiles[doc.metadata.source as string] = {
					path: doc.metadata.source as string,
		...
		if (appliedFilters && appliedFilters.length > 0) {
			const filterDescriptions = appliedFilters.map(filter => {
				if (filter.field === "modifiedAt" || filter.field === "createdAt") {
					return `${filter.field === "modifiedAt" ? "modified" : "created"} ${filter.condition} ${filter.value}`;
				} else {
		...
		return `${filterSummary}Filtered documents include: ${fileNames.join(", ")} (${docs.length} total chunks from ${Object.keys(uniqueFiles).length} files)`;
	}
    ...
    async processQueryWithHistory(
        ...
        if (!this.isInitialized || !this.allDocumentChunks) { 
            new Notice("RAG Service is not initialized. Please try again or re-initialize.");
            if (this.plugin) this.plugin.updateStatusBar("RAG: Not Initialized");
            return null; 
        }
        if ((!semanticQuery || semanticQuery.trim() === "") && (!metadataFilters || metadataFilters.length === 0)) {
            new Notice("Query or filter cannot be empty."); return null;
        }

        if (this.plugin) this.plugin.updateStatusBar("RAG: Processing...");
        ...
        if (metadataFilters && metadataFilters.length > 0) {
            ...
            for (const filter of metadataFilters) {
                if (filter.field === "modifiedAt" || filter.field === "createdAt") {
                    if (!primaryDateFilterField) primaryDateFilterField = filter.field;
                    const dateRange = getDateRangeForQuery(filter.condition, filter.value);
                    if (dateRange) {
                        filteredDocuments = filteredDocuments.filter(doc => {
                            const timestamp = doc.metadata[filter.field] as number; 
            ...
            new Notice(`Found ${filteredDocuments.length} chunks after filtering.`);
        ...
        if (!semanticQuery || semanticQuery.trim() === "") {
            if (this.plugin) this.plugin.updateStatusBar("RAG: Ready");
            const displayResult = this.formatResultsForDisplay(filteredDocuments, primaryDateFilterField);
            
            // Add filtered documents to conversation context for future queries
            const contextMessage = this.createContextFromFilteredDocuments(filteredDocuments, metadataFilters);
            if (contextMessage) {
        ...
        if (metadataFilters && metadataFilters.length > 0 && filteredDocuments.length < this.allDocumentChunks.length) {
            new Notice(`Performing semantic search on ${filteredDocuments.length} filtered chunks.`);
            ...
            for (const chunk of filteredDocuments) {
                const foundVector = allStoreVectors.find(sv => 
            ...
            if (filteredVectors.length > 0) {
                await tempVectorStore.addVectors(filteredVectors, filteredDocsForStore);
                currentChain = await createConversationalRAGChain(this.llm, tempVectorStore);
            } else if (filteredDocuments.length > 0) {
                new Notice("Could not map filtered chunks to existing vectors. Using original vector store.");
                currentChain = this.chain;
            }
        }

        if (!currentChain) {
            return "Error: RAG chain could not be initialized.";
        }
        ...
        try {
            ...
            if (metadataFilters && metadataFilters.length > 0) {
                const contextInfo = this.createContextFromFilteredDocuments(filteredDocuments, metadataFilters);
                let systemContext = "";
                
                // Check if query contains [[filename]] syntax and add system instructions
                if (semanticQuery.includes("[[") && semanticQuery.includes("]]")) {
                    systemContext += "Note: The user is using [[filename]] syntax to reference specific files. When you see [[filename]], it refers to the content of that specific file. ";
                }
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error("Error processing query with RAG chain:", errorMessage); 
    ...
    async reInitialize(): Promise<void> {
        ...
        this.llm = null;
        if (this.plugin) this.plugin.updateStatusBar("RAG: Re-initializing...");
        ...
        try {
            ...
            console.warn("Could not remove old vector store file during re-initialization:", e);
        }
