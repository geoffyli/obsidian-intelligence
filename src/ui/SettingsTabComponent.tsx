...
const SettingsTabComponent: React.FC<SettingsTabComponentProps> = ({ plugin }) => {
  ...
  const handleMySettingChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!plugin || !plugin.settings) return;
  ...
  if (!plugin || !plugin.settings) {
    return (
      <div className="rag-settings-container">
        <p className="text-center text-text-muted">Loading settings...</p>
      </div>
    );
  }
  ...
  return (
    <div className="rag-settings-container">
      <div className="rag-settings-field">
        <label htmlFor="openai-api-key" className="rag-settings-label">
          OpenAI API Key
        </label>
        <input
          id="openai-api-key"
