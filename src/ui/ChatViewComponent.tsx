...
const MessageRenderer: React.FC<MessageRendererProps> = ({ message, app, plugin }) => {
  ...
  useEffect(() => {
    if (messageRef.current) {
      ...
      const internalLinks = messageRef.current.querySelectorAll('a.internal-link');
...
const ChatViewComponent: React.FC<ChatViewComponentProps> = ({ plugin, app }) => {
  ...
  const [selectedFilterType, setSelectedFilterType] = useState<FilterSignature | null>(null);
  ...
  const getFileSuggestions = useCallback((query = ''): TFile[] => {
  ...
  const showFilterTypeSuggestions = useCallback((query: string) => {
  ...
  const showFileSuggestions = useCallback((query: string) => {
  ...
  const showFilterValueSuggestions = useCallback((filterSig: FilterSignature) => {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    const { semanticQuery, metadataFilters } = parseFiltersFromPrompt(rawInputText);

    addMessageToDisplay({ sender: 'user', text: rawInputText });

    if (semanticQuery) {
      setChatHistory((prev) => [...prev, { type: 'human', content: semanticQuery }]);
    }
    ...
    try {
      ...
      const aiResponseText = await plugin.ragService.processQueryWithHistory(
        semanticQuery,
        historyForChain,
        metadataFilters
      );
      ...
      if (aiResponseText) {
        ...
        if (semanticQuery) {
          setChatHistory((prev) => [...prev, { type: 'ai', content: aiResponseText }]);
        }
      } else {
        addMessageToDisplay({
          sender: 'system',
  ...
  const navigateSuggestions = useCallback((direction: number) => {
    ...
    setActiveSuggestionIndex(
      (prevIndex) =>
        (prevIndex + direction + currSuggestions.length) % currSuggestions.length
    );
  ...
  const selectActiveSuggestion = useCallback(() => {
    ...
    if (selectedSuggestion.type === 'filterType') {
      ...
      const textBeforeReplacement = inputValue.substring(0, suggestionQueryPos);
      const textAfterQuery = inputValue.substring(
        suggestionQueryPos + suggestionQuery.length
      );

      setInputValue(
        textBeforeReplacement + replacementText + ' ' + textAfterQuery.trimStart()
      );

      const newCursorPos = suggestionQueryPos + replacementText.length - 1;
      setTimeout(() => {
        if (inputAreaRef.current) {
          inputAreaRef.current.setSelectionRange(newCursorPos, newCursorPos);
          inputAreaRef.current.focus();
        }
      }, 0);
      ...
      let startReplacePos = inputValue.lastIndexOf('{', cursorPos - 1) + 1;
      ...
      setInputValue(
        textBeforeTrigger + `[[${file.basename}]]` + textAfterCursor.trimStart()
      );
  ...
  }, [isSuggesting, navigateSuggestions, selectActiveSuggestion, hideSuggestions, handleSendMessage]);
      ...
      >
        {uiMessages.map((message) => (
          ...
          >
            ...
            >
              <MessageRenderer message={message} app={app} plugin={plugin} />
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-stretch gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              aria-haspopup={isSuggesting ? 'listbox' : 'false'}
              aria-controls={isSuggesting ? 'suggestion-popover-list' : undefined}
            ...
            {isSuggesting && currSuggestions.length > 0 && (
              ...
              >
                {currSuggestions.map((suggestion, index) => (
                  <button
